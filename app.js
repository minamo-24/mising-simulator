...
const NPC_DEBUFF_WAIT_TIME = 2.5;
...
const SPREAD_METHODS = {
  ...
  piren: {
    name: "ぴれん式",
    description: "図を基準に、奇数回は塔周辺の縦配置、偶数回は左右対称の上下配置で処理します。塔内でデバフが被った場合、塔後ろにいた人が調整します。",
  },
};
...
function pirenAssignmentFor(player, round) {
  ...
  const mark = markForRound(player, round);
  if (info.odd) {
    if (mark === "fan") return { tower: 0, x: 300, y: 555, name: "塔1・左誘導扇" };
    if (mark === "circle") return { tower: 1, x: 500, y: 560, name: "塔2・下円" };
    if (
      [3,5,7].includes(round) &&
      player.adjustRound === round
    ) {
    let side = player.towerSide;
    if (player.swapSide) {
      side = side === 0 ? 1 : 0;
    }
    return side === 0
  ...
  let side;
  if (player.adjustRound === round) {
    side = player.towerSide;
    if (player.swapSide) {
      side = side === 0 ? 1 : 0;
    }
  } else {
    side = markSide(player, round);
  }
  if (mark === "fan") {
    return side === 0
...
function stagingPositionFor(player, round) {
  ...
  // 偶数回で調整情報がある場合、
  // いきなり塔位置へ行かず、まず未来/過去誘導位置に集合する
  if (
    round % 2 === 0 &&
    player.adjustRound === round
  ) {
    return stackPositionFor(round);
  }
...
function npcTarget(player) {
  ...
  if (
    player.adjustRound === round &&
    state.time < player.adjustReadyAt
  ) {
...
function resolveTower(round) {
  ...
  const next = active
    .map(member => nextRoundFor(member, round))
    .find(Boolean);
...
function calculateSwap(targetRound) {
  const info = towerInfo(targetRound);
  const active = state.players.filter(
    p => p.group === info.group
  );

  for (const player of active) {
    player.swapSide = false;
    player.adjustRound = targetRound;
    player.adjustReadyAt = state.time + NPC_DEBUFF_WAIT_TIME;
  }

  const leftTower = active.filter(
    p => distance(p, TOWERS[0]) <= TOWERS[0].r
  );
  const rightTower = active.filter(
    p => distance(p, TOWERS[1]) <= TOWERS[1].r
  );
  ...
  rightTower.forEach(p => {
    p.towerSide = 1;
  });

  for (const tower of [leftTower, rightTower]) {
    const fanPlayers = tower.filter(
      p => markForRound(p, targetRound) === "fan"
    );

    const circlePlayers = tower.filter(
      p => markForRound(p, targetRound) === "circle"
    );
    const sharePlayers = tower.filter(
      p => markForRound(p, targetRound) === "share"
    );
    // 偶数回：扇扇 or 円円なら、塔内で一番後ろを逆側へ
    if (
      targetRound % 2 === 0 &&
      (fanPlayers.length === 2 || circlePlayers.length === 2)
    ) {
      const backPlayer = [...tower]
        .sort((a, b) => b.y - a.y)[0];
      backPlayer.swapSide = true;
    }
    // 3,5,7回目：頭割り頭割りなら、頭割りの後ろを逆側へ
    if (
      [3, 5, 7].includes(targetRound) &&
      sharePlayers.length === 2
    ) {
      const backShare = [...sharePlayers]
        .sort((a, b) => b.y - a.y)[0];
      backShare.swapSide = true;
    }
    for (const player of active) {
      if (player.towerSide === null) {
        player.adjustRound = null;
        player.swapSide = false;
      }
    }
  }
